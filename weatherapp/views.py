...
class CityDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['weather'] = pyreq.get(
            api_url.format(context['city'].name,units,config('forecast_API_KEY'))).json()

        context['coords_available'] = not(
            context['city'].latitude is None or context['city'].longitude is None)

        ts_list = [ts['dt'] for ts in context['weather']['list']]
        context['dates'] = [
            datetime.datetime.utcfromtimestamp(ts) for ts in ts_list]
